# main.py — Bassam AI (Render-ready)
...
<title>نتيجة بسام الذكي</title>
<body style="font-family:Segoe UI,Tahoma,Arial,sans-serif;direction:rtl;padding:20px;background:#f8fafc">
  <div style="max-width:800px;margin:auto;background:#fff;border-radius:14px;padding:24px;box-shadow:0 10px 30px rgba(0,0,0,.08)">
    <h2>🔎 نتيجتك</h2>
    <p><b>الوضع:</b> {mode}</p>
    <p><b>النص المدخل:</b> {query}</p>
    <hr style="margin:16px 0">
    <p>هذا رد مبدئي للتأكيد أن المسارات تعمل. سنربط المنطق الحقيقي لاحقًا.</p>
    <p><a href="/" style="color:#4f46e5">⬅ الرجوع للصفحة الرئيسية</a></p>
  </div>
</body>
</html>
"""
...
async def home():
    """الصفحة الرئيسية مع واجهة HTML"""
    return HTMLResponse(content=HOME_HTML)

@app.post("/search", response_class=HTMLResponse)
async def search(query: str = Form(...), mode: str = Form("smart")):
    """معالجة نموذج البحث (مؤقتًا نعرض النتيجة فقط)"""
    safe_q = html.escape(query)
    safe_mode = html.escape(mode)
    return HTMLResponse(RESULT_HTML_TMPL.format(query=safe_q, mode=safe_mode))
...
async def upload_page():
    ...
    <body style="font-family:Segoe UI,Tahoma,Arial,sans-serif;direction:rtl;padding:20px">
      <h2>📷 رفع صورة (تجريبي)</h2>
      <p>سنضيف رفع الصورة ومعالجة المسألة لاحقًا.</p>
      <p><a href="/">⬅ الرجوع</a></p>
    </body></html>
    """)
