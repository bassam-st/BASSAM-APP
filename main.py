...
# ===== واجهة HTML افتراضية (تشبه ChatGPT) =====
BASIC_HTML = """
<!doctype html>
<html lang="ar" dir="rtl">
<meta charset="utf-8">
<title>Bassam — v3.1</title>
...
.chat{height:58vh;overflow:auto;padding:14px}
.msg{padding:10px 12px;border-radius:12px;margin:8px 0;line-height:1.6;white-space:pre-wrap}
.user{background:#152043;border:1px solid #24336b;align-self:flex-end}
...
<div class="container">
  <div class="header">
    <h2 style="margin:0">🤖 بسّام — Omni Brain</h2>
    <span class="badge">v3.1</span>
  </div>

  <div class="card" style="margin-top:16px">
    <div id="chat" class="chat">
      <div class="msg bot">أهلًا! اكتب سؤالك بالعربي وسأحاول مساعدتك 🌟</div>
    </div>
    <div class="row">
      <input id="q" placeholder="اكتب سؤالك… ثم اضغط إرسال">
      <button id="send">إرسال</button>
    </div>
  </div>

  <p class="hint">
    نصائح: اسأل عن الرياضيات البسيطة، ويكيبيديا، البحث من ملفاتك (إن وجدت فهارس RAG)، أو دعني ألخص لك شيئًا من الويب.
  </p>

  <div class="card" style="margin-top:18px;padding:14px">
    <b>نقاط الدخول (API):</b>
    <ul>
      <li><code>GET /healthz</code> — فحص الصحة</li>
      <li><code>GET /ask?q=...</code> — إجابة فورية (JSON)</li>
      <li><code>POST /chat</code> — محادثة (Body JSON: {"message":"..."})</li>
    </ul>
  </div>
</div>
...
async function ask(){
  ...
  try{
    ...
    push('bot', '⚠️ حدث خطأ أثناء الاتصال بالخادم.');
  }finally{
    send.disabled = false;
    q.focus();
  }
...
async def home(request: Request):
    if templates:
        # مع إصدارات Starlette الحديثة: TemplateResponse(request, name, context)
...
# زر "ابدأ" من نموذج بسيط (لو استخدمت صفحة index كلاسيكية)
@app.post("/search")
...
# تشغيل محليًا أو عند بعض المنصات
